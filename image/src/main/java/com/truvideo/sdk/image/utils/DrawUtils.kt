...
internal object DrawUtils {
    ...
    fun createTextImage(
        ...
        val bitmap = createBitmap(textWidth.toInt(), textHeight.toInt())
    ...
    fun mergeImages(imageSize: Size, images: List<DrawingData>): Bitmap {
        val bitmap = createBitmap(imageSize.width.toInt(), imageSize.height.toInt())
        val canvas = Canvas(bitmap)
        ...
        for (imageData in images) {
            ...
            canvas.withTranslation(x, y) {
                scale(imageData.scale, imageData.scale, w * 0.5f, h * 0.5f)
                rotate(imageData.rotation, w * 0.5f, h * 0.5f)
                drawBitmap(
                    imageBitmap,
